...
- (void)drawRect:(CGRect)rect {
    ...
    [self drowImage:_centerImg];
}
- (void)drowImage:(UIImage *)image {
    CGFloat radius = MIN(self.bounds.size.height, self.bounds.size.width) * 0.5;
    image = [UIImage createRoundedRectImage:image size:image.size radius:image.size.height/2];
    [image drawInRect:CGRectMake(radius* (0.1+0.075),radius* (0.1+0.075),radius*(0.9-0.075)*2, radius*(0.9-0.075)*2)];
}
/**
 *  绘制文字
 */
