...
-(void)initwithtitle:(NSString *)str andmessage:(NSString *)str1 andcancelbtn:(NSString *)cancel andotherbtn:(NSString *)other
{
    ...
    self.datePicker.frame = CGRectMake(0, 20, self.frame.size.width-20, 200);
...
-(void)selectbtn:(id)sender
{
    ...
    if (sure_date&&sure_date.length>0) {
        [self.delegate btnindex:(int)btn.tag :(int)self.tag withSureDate:sure_date];
    }else {
         [self.datePicker pickerView:self.datePicker didSelectRow:0 inComponent:0    ];
        [self.delegate btnindex:(int)btn.tag :(int)self.tag withSureDate:sure_date];
    }
